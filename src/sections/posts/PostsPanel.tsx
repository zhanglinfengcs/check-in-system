...
import { PostType } from '../../_mock/post-list';
...
interface EnhancedTableProps {
  numSelected: number;
  onRequestSort: (event: React.MouseEvent<unknown>, newOrderBy: keyof PostType) => void;
...
export default function PostsTable({props}: {props: PostType[]}) {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof PostType>(DEFAULT_ORDER_BY);
  ...
  const handleRequestSort = React.useCallback(
    (event: React.MouseEvent<unknown>, newOrderBy: keyof PostType) => {
      const isAsc = orderBy === newOrderBy && order === 'asc';
      const toggledOrder = isAsc ? 'desc' : 'asc';
      setOrder(toggledOrder);
      setOrderBy(newOrderBy);

      const sortedRows = stableSort(postList, getComparator(toggledOrder, newOrderBy));
  ...
  const handleChangePage = React.useCallback(
    (event: unknown, newPage: number) => {
      ...
      const numEmptyRows =
        newPage > 0 ? Math.max(0, (1 + newPage) * rowsPerPage - postList.length) : 0;
    },
